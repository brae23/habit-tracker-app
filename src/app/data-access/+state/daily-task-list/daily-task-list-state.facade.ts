...
export class DailyTaskListStateFacade {
  @Select(DailyTaskListState.getDailyTaskList)
  dailyTaskList$!: Observable<TaskList>;
  ...
  addInsetListItem(listItem: IListItem, parentListId: string) {
    this.store.dispatch(
      new DailyTaskListActions.AddInsetListItem(listItem, parentListId),
    );
  }
  ...
  removeInsetListItem(listItemId: string, parentListId: string) {
    this.store.dispatch(
      new DailyTaskListActions.RemoveInsetListItem(listItemId, parentListId),
    );
  }

  updateListCollapsedState(listItemId: string, collapsedState: boolean) {
    this.store.dispatch(
      new DailyTaskListActions.UpdateListCollapsedState(
        listItemId,
        collapsedState,
      ),
    );
  ...
  updateListCompletedState(listItemId: string, completedState: boolean) {
    this.store.dispatch(
      new DailyTaskListActions.UpdateListCompletedState(
        listItemId,
        completedState,
      ),
    );
